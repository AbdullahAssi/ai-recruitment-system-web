...
async function testResumeExtraction(resume: any) {
  ...
  const response = await fetch(
    "http://localhost:8000/api/v1/scoring/score-by-id",
    {
      method: "POST",
      ...
      body: JSON.stringify({
        resume_id: resume.id,
        job_description:
          "Looking for a Full Stack Developer with React, Node.js, TypeScript, Python. 3+ years experience.",
